...
async def assistant_chat_websocket(websocket: WebSocket, project_name: str):
    ...
    - Help plan and break down features
    - Create features in the database (via feature_create_bulk)
    - Read and analyze code
    - Search the codebase

    Cannot modify files directly (use coding agent for implementation).

    Message protocol:

    Client -> Server:
    - {"type": "start", "conversation_id": int | null} - Start/resume session
    - {"type": "message", "content": "...", "attachments": [...]} - Send user message with optional images
        ...
        while True:
            try:
                ...
                if msg_type == "ping":
                    ...
                    async for chunk in session.send_message(user_content, attachments):
                        await websocket.send_json(chunk)
