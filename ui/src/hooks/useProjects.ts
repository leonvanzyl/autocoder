...
export function useStartAgent(projectName: string) {
  ...
  return useMutation({
    mutationFn: (yoloMode: boolean = false) =>
      api.startAgent(projectName, yoloMode),
    onSuccess: () => {
...
export function useKnowledgeFiles(projectName: string | null) {
  return useQuery({
    ...
    queryFn: () => api.listKnowledgeFiles(projectName!),
    enabled: !!projectName,
  });
}
...
export function useKnowledgeFile(
  ...
  return useQuery({
    ...
    queryFn: () => api.getKnowledgeFile(projectName!, filename!),
    enabled: !!projectName && !!filename,
  });
}

export function useUploadKnowledgeFile(projectName: string) {
  const queryClient = useQueryClient();
  ...
  return useMutation({
    mutationFn: ({
      ...
      filename: string;
      content: string;
    }) => api.uploadKnowledgeFile(projectName, filename, content),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["knowledge-files", projectName],
      });
...
export function useDeleteKnowledgeFile(projectName: string) {
  ...
  return useMutation({
    mutationFn: (filename: string) =>
      api.deleteKnowledgeFile(projectName, filename),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["knowledge-files", projectName],
      });
