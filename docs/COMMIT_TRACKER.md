...
| `bf3a6b0` | Per‑agent log viewer + stuck agent fixes + scheduling score | **Partially ported** | Added per‑agent log jump (Agent Status → logs drawer) + friendlier agent status labels. We already have WorkerLogsPanel + gatekeeper artifacts + retry/backoff. Scheduling score not applicable. |
