...
Usage (Windows / macOS / Linux):
  python scripts/e2e_qa_provider.py --out-dir "G:/Apps/autocoder-e2e-fixtures" --provider multi_cli
"""
...
""".lstrip(),
    ...
    feature_id = int(db.create_feature("E2E: QA provider fixes Gatekeeper failure", "Deterministic failure", "testing"))
...
""".lstrip(),
    ...
    if db_path.exists():
        db_path.unlink()
    db = Database(str(db_path))
    feature_id = int(db.create_feature("E2E: QA provider fixes pytest failure", "Deterministic pytest failure", "testing"))
...
def run_e2e(project_dir: Path, *, provider: str, timeout_s: int) -> int:
    os.environ["AUTOCODER_REQUIRE_GATEKEEPER"] = "1"
    os.environ["AUTOCODER_QA_SUBAGENT_ENABLED"] = "1"
    os.environ["AUTOCODER_QA_SUBAGENT_PROVIDER"] = provider
...
def main() -> int:
    ...
    ap.add_argument("--provider", default="multi_cli", choices=["claude", "codex_cli", "gemini_cli", "multi_cli"])
    ...
    if args.provider in {"codex_cli", "multi_cli"} and shutil.which("codex") is None:
        print("codex CLI not found; install it or pick --provider gemini_cli/claude", file=sys.stderr)
        return 4
    if args.provider in {"gemini_cli", "multi_cli"} and shutil.which("gemini") is None:
        print("gemini CLI not found; install it or pick --provider codex_cli/claude", file=sys.stderr)
        return 4

    return run_e2e(project_dir, provider=args.provider, timeout_s=int(args.timeout_s))
